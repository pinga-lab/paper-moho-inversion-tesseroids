...
\noindent in which $d_i$ is the $i$th element of the predicted data vector
$\mbf{d}$, $\mbf{p}$ is the parameter vector containing the Moho depths
($z_k$),
and
$f_i$ is the $i$th non-linear function that maps the parameters onto the data.
...
methods like the Gauss-Newton method or the Steepest Descent method.
...
for the Gauss-Newton method is
...
\begin{equation}
    \mbf{H}(\mbf{p}^k) = 2\mbf{A}(\mbf{p}^k)^T\mbf{A}(\mbf{p}^k) +
    2\mu\mbf{R}^T\mbf{R},
\end{equation}

\noindent
$\mbf{\nabla}\Gamma$ is the gradient vector of $\Gamma$,

\begin{equation}
    \mbf{\nabla}\Gamma(\mbf{p}^k) = -2\mbf{A}(\mbf{p}^k)^T[\mbf{d}^o - \mbf{d}] +
...
\begin{equation}
    A_{ij} = \dfrac{\partial f_i}{\partial p_j}(\mbf{p}).
\end{equation}
