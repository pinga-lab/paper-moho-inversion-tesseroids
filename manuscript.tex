...
Bott's method requires the same number of observed data ($N$)
and model parameters ($M$).
Limited ways to separate the training and testing data sets.
Data and model must be on a regular grid.
Cannot randomly separate.
Use a dense data grid.
Take for the training set a sub grid with double the original grid spacing.
Every other point is included in the testing data set.
The testing data set will have more points than training data set.
One can generate observed data on a grid with half of the desired grid spacing.
This is easy for data generated from spherical harmonic models.
